...
const TDSSummary: React.FC<TDSSummaryProps> = ({ profile, onApply, onDiscard, onSave }) => {
    ...
    const { translateData, language } = useDataTranslation(); // Keep language for specific logic if needed
    ...
    const getLabel = (attrId: string) => {
        ...
        return attr ? translateData({ en: attr.nameEn, es: attr.nameEs }) : attrId;
    };
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-gradient-to-r from-brand-600 to-brand-700 px-6 py-4">
                <h3 className="text-white font-bold text-lg">
                    {t('tds.analysis')}
                    <span className="text-brand-200 text-sm ml-2">
                        ({profile.mode === 'normal' ? t('tds.modeNormal') : t('tds.modeExpert')})
                    </span>
                </h3>
            </div>
                ...
                <div className="p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-2">
                        ...
                        <span className="text-xs text-gray-500">
                            {t('tds.duration')}: {profile.totalDuration.toFixed(1)}s | {t('tds.swallowVerb')}: {profile.swallowTime.toFixed(1)}s
                        </span>
                    </div>
                                            ...
                                            {profile.events.map((event, idx) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        aria-label={`${getLabel(event.attrId)}: ${event.start.toFixed(1)}s - ${event.end.toFixed(1)}s`}
                                                    />
                                                );
                                            })}
                                        </div>
                            ...
                            <div className="px-3 py-1.5 bg-gray-100 rounded-lg flex items-center gap-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-medium text-gray-800 text-sm">
                                        {getLabel(profile.events[selectedEventIdx].attrId)}
                                    </span>
                                </div>
                        ...
                        >
                            <div className="flex items-center justify-between w-full">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-gray-700 font-medium">{getLabel(attrId)}</span>
                                    {result.isFlagged && (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`font-bold w-6 text-right ${result.boostDetails ? 'text-indigo-700' : 'text-brand-800'}`}>{result.score}</span>
                                </div>
                            </div>
                            ...
                            {result.boostDetails && (
                                <div className="mt-1 ml-5 text-xs text-indigo-600 flex items-start gap-1">
                                    ...
                                    <span>
                                        {` ${t('tds.recommendation')}: +${result.boostDetails.amount} (${result.boostDetails.reason === 'aftertaste'
                                            ? `${t('tds.lingeringPresence')}: ${result.boostDetails.duration}s`
                                            : result.boostDetails.reason === 'mixed'
                                                ? `${t('tds.strongPresence')}: ${result.boostDetails.duration}s`
                                                : `${t('tds.componentPresence')}: ${result.boostDetails.duration}s`
                                            })`}
                                    </span>
                                </div>
                            )}
                            ...
                            {complementaryScores.map(([attrId, result]) => (
                                ...
                                >
                                    <div className="flex items-center justify-between w-full">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-gray-400">{result.durationPercent}%</span>
                                            <span className={`font-bold w-6 text-right ${result.boostDetails ? 'text-indigo-700' : 'text-brand-700'}`}>{result.score}</span>
                                        </div>
                                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border-y">
                ...
                <div className="bg-white/70 rounded-lg p-3 backdrop-blur">
                    <div className="flex items-center gap-2 text-orange-700 mb-1">
                        <Sparkles size={16} />
                        <span className="text-xs font-bold uppercase">{t('tds.aftertaste')}</span>
                    </div>
                    ...
                    <div className={`text-xs mt-1 ${analysis.aftertasteQuality === 'positive' ? 'text-green-600' :
                        ...
                        {analysis.aftertasteQuality === 'positive' ? t('tds.positive') : analysis.aftertasteQuality === 'negative' ? t('tds.negative') : t('tds.neutral')}
                    </div>
                </div>
            </div>
                ...
                <div className="px-4 py-2 bg-amber-50 border-b border-amber-100">
                    {hasFlaggedScores && (
                        <div className="flex items-center gap-2 text-amber-700 text-sm">
                            ...
                            <span>{t('tds.coreNotSelected')}</span>
                        </div>
                    )}
                </div>
            )}
